...
after update 
	...
	select @id_per=  Id_perfil,@fecha_cierre_per = Fecha_cierre ,
	@monto = Precio from Precios_Perfil
	------variables para comparar reserva con precios perfil 
	declare @id_reper int, @fecha_final_rese date,
	@id_cliente int, @id_reser int
  
	select @id_reser = Id_reserva,@id_cliente= Id_clientes , @id_reper=Id_perfil,
	@fecha_final_rese= Fecha_final from Reserva 
     
	 ------------join para tomar id vehiculo y insertarla en la tabla renta como corresponde una variable mas para guardar la cantidad del monto final si se pasa de la fecha

	 declare @id_vehiculo int, @dias_atraso int, @monto2fin float
	 	select @dias_atraso=DATEDIFF(DAY,@fecha_final_rese,GETDATE()) from Reserva
	 select @id_vehiculo=Id_vehiculo from Precios_Perfil p join vehiculo v on p.Id_perfil = v.Id_perfil 
	 where v.Id_perfil = @id_reper
	...
	begin 
		...
		values (14,@id_reser,@id_cliente,@id_vehiculo,@monto)
		 ...
		 set @monto2fin = @dias_atraso*1500 + @monto
		insert into Renta (Id_renta,Id_reserva,Id_clientes,Id_vehiculo,Total_pago)
		values (14,@id_reser,@id_cliente,@id_vehiculo,@monto2fin)
		end
	end
	
	update Reserva 
	set Fecha_final = '2021/11/15'
	where Id_reserva = 8
